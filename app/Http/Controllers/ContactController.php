...
class ContactController extends Controller
{
    ...
    public function messageForUser(Request $request){
       ...
       $subject= 'Campaign form';
       
        $message=str_replace('{{email}}',$contact['email'],$message);
        $message=str_replace('{{phone}}',$contact['phone'],$message);
        $message=str_replace('{{name}}'," ",$message);
        $message=str_replace('{{subject}}','Interested by the project',$message);
        $message=str_replace('{{message}}',$contact['bedroom'].' bedrooms',$message);

        Mail::to($setting->email)->send(new ContactMessageInformation($message, "Campaign Form"));
